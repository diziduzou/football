...
var jadeUrl = ['./views/**/*.jade', '!./views/common/*.jade'];
var jadeDest = './build/';
...
gulp.task('watch-compile', function() {
    ...
    watch(coffeeUrl, function(event) {
        return gulp.src(coffeeUrl)
            .pipe(sourcemaps.init())
            .pipe(plumber())
            .pipe(changed(coffeeDest, {
                extension: '.min.js'
            }))
            .pipe(coffee({
                bare: true
            })).on('error', function(err) {
                gutil.log(gutil.colors.red('coffee compile error!\n') + err);
            })
            .pipe(uglify())
            .pipe(rename({
                suffix: '.min'
    ...
    watch(jsUrl, function() {
        return gulp.src(jsUrl)
            ...
            .pipe(uglify()).on('error', function(err) {
                gutil.log(gutil.colors.red('js compress error!\n') + err);
            })
            .pipe(rename({
                suffix: '.min'
            }))
...
gulp.task('watch-compile-server-reload', function() {
    ...
    watch(coffeeUrl, function() {
        return gulp.src(coffeeUrl)
            ...
            .pipe(uglify()).on('error', function(err) {
                gutil.log(gutil.colors.red('coffee compile js compress error!\n') + err);
            })
            .pipe(rename({
                suffix: '.min'
            }))
...
gulp.task('compile-coffee', function() {
    return gulp.src(coffeeUrl)
        ...
        .pipe(coffee()).on('error', function(err) {
            gutil.log(gutil.colors.red('init coffee compile error!\nplease fix it!') + err);
        })
        .pipe(uglify())
        .pipe(rename({
            suffix: '.min'
